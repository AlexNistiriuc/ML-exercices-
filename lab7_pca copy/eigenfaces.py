...
class Eigenfaces:
    ...
    def fit(self, X: np.ndarray, verbose: bool = False):
        ...
        cov = np.einsum('ij,kj->ik', X_norm, X_norm, optimize=True)
        ...
        self._ef = np.einsum('ij,ik->jk', X_norm, principal_sorted_eigvec, optimize=True)

        # normalize the retrieved eigenvectors to have unit length
        self._ef = self._ef / np.sqrt(np.maximum(principal_sorted_eigval, np.finfo(float).eps))
    ...
    def transform(self, X: np.ndarray):
        ...
        X_norm = X - self._x_mean
        return np.einsum('ij,jk->ik', X_norm, self._ef, optimize=True)

    def inverse_transform(self, X: np.ndarray):
        return np.einsum('ij,kj->ik', X, self._ef, optimize=True) + self._x_mean
...
def main():
    ...
    # get_data
    dataset_path = Path(__file__).with_name('att_faces')
    X_train, Y_train, X_test, Y_test = get_faces_dataset(path=str(dataset_path), train_split=0.6)
