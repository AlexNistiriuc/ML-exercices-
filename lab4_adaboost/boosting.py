...
class WeakClassifier:
    ...
    def fit(self, X: np.ndarray, Y: np.ndarray):
        ...
        self._threshold = np.random.uniform(min=np.min(X[:, self._dim]), max=np.max(X[:, self._dim]))
...
class AdaBoostClassifier:
    ...
    def fit(self, X: np.ndarray, Y: np.ndarray, verbose: bool = False):
        ...
        for l in range(self.n_learners):
            ...
            cur_idx = np.random.choice(n, size=int(n/2), p=sample_weights)
    ...
    def predict(self, X: np.ndarray):
        ...
        for i in range(num_samples):
            for j in range(self.n_learners):
                y_pred += self.alphas[j] * self.learners[j].predict(X[i:i+1])
