...
class SVM:
    ...
    def loss(self, y_true, y_pred):
        ...
        return 0.5 * norm(self._w) **2 + np.mean(np.maximum(0, 1 - y_true*y_pred))  
    ...
    def fit_gd(self, X, Y, verbose=False):
        ...
        for e in range(1, self._n_epochs+1):
            for j in range(n_samples):
                t += 1
                learning_rate = 1 / (t * self._lambda + eps)
                dw = self._lambda * self._w
    ...
    def predict(self, X):
        ...
        """
        return np.where(np.dot(self._w, X) > 0.0,
